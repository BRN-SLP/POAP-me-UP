...
import { createLogger, isLogLevel, setDefaultLogLevel, promptToSelectMultiple, promptForText, pluralizeNoun, promptToContinue, printBoolean, printJson, isFile } from '@layerzerolabs/io-devtools';
import { printLogo, render, createProgressBar, printRecords } from '@layerzerolabs/io-devtools/swag';
import { formatEid, createSignAndSendFlow, splitCommaSeparated, isDeepEqual, serializeDockerComposeSpec } from '@layerzerolabs/devtools';
...
import 'hardhat/internal/core/config/config-loading';
...
import { EndpointId, endpointIdToStage, Environment as Environment$1, Stage } from '@layerzerolabs/lz-definitions';
...
var action = async ({ networks: networksArgument, tags: tagsArgument = [], logLevel: logLevel2 = "info", ci = false, reset = false, stage: stage2 }, hre) => {
  ...
  const filteredEidsByNetworks = stage2 == null ? eidsByNetworks : eidsByNetworks.filter(([, eid2]) => eid2 != null && endpointIdToStage(eid2) === stage2);
  ...
  if (isInteractive) {
    ...
    const options = eidsByNetworks.map(([networkName, eid2]) => ({
      ...
      hint: eid2 == null ? void 0 : `Connected to ${formatEid(eid2)}`
    })).sort(
      (a, b) => (
  ...
  if (errors.length === 0) {
    return logger3.info(`${printBoolean(true)} Your contracts are now deployed`), results;
  }
  logger3.error(
...
if (process.env.LZ_ENABLE_EXPERIMENTAL_SIMULATION) {
  task(TASK_LZ_TEST_SIMULATION_LOGS, "Show logs for LayerZero omnichain simulation", action2).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel).addFlag("follow", "Follow log output");
}
...
var action3 = async ({ networks: networksArgument, daemon = false, logLevel: logLevel2 = "info", stage: stage2 }, hre) => {
  ...
  const isOnStage = stage2 == null ? () => true : (eid2) => endpointIdToStage(eid2) === stage2;
...
subtask(
  ...
  ({ transactions, ...args }) => createSignAndSendFlow(args)({ transactions })
...
task(TASK_LZ_EXPORT_DEPLOYMENTS_TYPESCRIPT, "Export deployments as TypeScript files", action5).addParam(
  "networks",
  "List of comma-separated networks. If not provided, all networks will be deployed",
  void 0,
  types.csv,
...
task(TASK_LZ_VALIDATE_SAFE_CONFIGS, "Validate safe configs in hardhat.config.ts", action6);
var RPC_URL_KEY = "url";
var HTTP_URL = "http://";
var HTTPS_URL = "https://";
var WS_URL = "ws://";
...
var checkRpcHealth = async (provider, networkName, timeout, retries = RETRIES, retryDelay = RETRY_DELAY) => {
  let errorMessage = "";
  for (let attempt = 1; attempt <= retries; ++attempt) {
    try {
      ...
      logger2.info(`Network ${networkName}'s RPC is healthy. Latest block number: ${blockNumber}`);
  ...
  logger2.error(`Network ${networkName} failed health check after ${retries} attempts.`);
...
var action7 = async (taskArgs, hre) => {
  ...
  const isOnStage = taskArgs.stage == null ? () => true : (eid2) => endpointIdToStage(eid2) === taskArgs.stage;
  ...
  logger2.info(
    `========== Validating RPC URLs for networks: ${((_a = taskArgs.networks) == null ? void 0 : _a.join(", ")) || Object.keys(networks).join(", ")}`
  );
  const networksWithInvalidRPCs = [];
  await Promise.all(
    Object.entries(networks).map(async ([networkName, networkConfig]) => {
      const eid2 = eidByNetworkName[networkName];
      if (!eid2) {
        logger2.info(`No eid found for network ${networkName}, skipping`);
        return;
      }
      const rpcUrl = networkConfig == null ? void 0 : networkConfig[RPC_URL_KEY];
      if (!rpcUrl) {
        logger2.error(`No RPC URL found for network ${networkName}, skipping`);
...
${networksWithInvalidRPCs.map((failure) => `${failure.networkName}: ${failure.errorMessage}`).join("\n")} `
    );
    if (taskArgs.continue) {
      logger2.warn("Continuing execution despite invalid RPC URLs due to --continue flag");
    } else {
      logger2.error("Invalid RPCs: you can set --continue flag to continue anyway");
      process.exit(1);
    }
  } else {
    logger2.info(`${printBoolean(true)} ========== All RPC URLs are valid!`);
  }
};
task(
  ...
  types$1.int,
  true
).addFlag("continue", "Continue even if some RPCs are invalid").addParam("networks", "Comma-separated list of networks to simulate", void 0, types.csv, true).addParam("stage", "Chain stage. One of: mainnet, testnet, sandbox", void 0, types.stage, true);
