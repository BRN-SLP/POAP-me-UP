...
import { types, createOmniGraphHardhatSchema, createOmniNodeHardhatSchema, createOmniEdgeHardhatSchema, SUBTASK_LZ_SIGN_AND_SEND, createErrorParser, getAllArtifacts, isErrorFragment, createConnectedContractFactory, createGnosisSignerFactory, createSignerFactory, getNetworkNameForEid, assertDefinedNetworks, getEidsByNetworkName, createOmniPointHardhatTransformer, createProviderFactory, getEidForNetworkName, formatOmniTransaction, createOmniGraphHardhatTransformer } from '@layerzerolabs/devtools-evm-hardhat';
import { RevertError, PanicError, CustomError } from '@layerzerolabs/devtools-evm';
import { printLogo, printRecord, printVerticalTable, printRecords } from '@layerzerolabs/io-devtools/swag';
import { setDefaultLogLevel, createLogger, createModuleLogger, printJson, printBoolean, printCrossTable, printRecord as printRecord$1, promptToSelectMultiple, pluralizeNoun } from '@layerzerolabs/io-devtools';
import { Fragment } from '@ethersproject/abi';
import { hexDataSlice, isBytes, hexlify, isHexString, arrayify, concat, zeroPad, hexZeroPad } from '@ethersproject/bytes';
import { id } from '@ethersproject/hash';
import { createConfigExecuteFlow, createWireFlow, areVectorsEqual, createDefaultApplicative, formatEid, isZero, createConfigLoadFlow, formatOmniPoint } from '@layerzerolabs/devtools';
import { OAppNodeConfigSchema, OAppEdgeConfigSchema, OAppReadNodeConfigSchema, OwnableNodeConfigSchema, configureOApp, checkOAppPeers, configureOAppRead, checkOAppEnforcedOptions, configureOwnable, CommandResolver, TimeMarkerResolver, TimeMarkerValidator } from '@layerzerolabs/ua-devtools';
import { writeFileSync } from 'fs';
import { createOAppFactory, createOAppReadFactory, createOwnableFactory, SingleViewFunctionCallerEVM, ComputerEVM, EVMTimeMarkerResolverChain, EVMTimeMarkerValidatorChain } from '@layerzerolabs/ua-devtools-evm';
...
import { createPrinter, createSourceFile, ScriptTarget, ScriptKind, ListFormat, factory, NodeFlags } from 'typescript';
...
var action = async ({ hash, logLevel = "info" }, hre) => {
  ...
  const logger4 = createLogger(logLevel);
  try {
...
task(TASK_LZ_ERRORS_DECODE, "Decodes custom error data based", action).addPositionalParam("hash", "Encoded contract error hash (including the 0x prefix)", void 0, types.string, false).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel);
var action2 = async ({ containing = "", showSourcePath = false, logLevel = "info" }, hre) => {
  ...
  const rows = artifacts.flatMap(
    (artifact) => artifact.abi.filter(isErrorFragment).map(Fragment.from).map((fragment) => ({
...
task(TASK_LZ_ERRORS_LIST, "List all custom errors from your project", action2).addParam("containing", "Only show custom errors containing a string", "", types.string).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel).addFlag("showSourcePath", "Show contract source path");
var OAppOmniGraphHardhatSchema = createOmniGraphHardhatSchema(
...
subtask(SUBTASK_LZ_OAPP_WIRE_CONFIGURE, "Create a list of OmniTransactions that configure your OApp", action3).addParam("graph", "Configuration graph", void 0, types.any).addParam("configurator", "Configuration function matching the SDK factory", void 0, types.any, true).addParam("sdkFactory", "SDK factory for an OmniSDK", void 0, types.any, true);
...
task(TASK_LZ_OAPP_WIRE, "Wire LayerZero OApp", action4).addParam("oappConfig", "Path to your LayerZero OApp config", void 0, types.string).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel).addParam(
  "loadConfigSubtask",
...
).addFlag("safe", "Use gnosis safe to sign transactions").addParam("signer", "Index or address of signer", void 0, types.signer, true).addParam("outputFilename", "Output filename for the generated transactions", void 0, types.string, true).addOptionalParam(
  "skipConnectionsFromEids",
  ...
  try {
    ...
    const peerNetworkMatrix = points.map((row) => {
      const connectionsForCurrentRow = points.reduce((tableRow, column) => {
        ...
        return {
          ...tableRow,
          [column.networkName]: printBoolean(connection == null ? void 0 : connection.hasPeer)
        };
      }, {});
    ...
    console.log(` ${printBoolean(void 0)} - Ignored`);
    return peers;
  } catch (error) {
    throw new Error(`An error occurred while getting the OApp configuration: ${error}`);
  }
};
task(TASK_LZ_OAPP_PEERS_GET, "Outputs OApp peer connections", action5).addParam("oappConfig", "Path to your LayerZero OApp config", void 0, types.string).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel);
...
async function getReadConfig(endpointV2Sdk, channelIds = [ChannelId.READ_CHANNEL_1], address, custom2 = false) {
  const availableChannels = Object.keys(ChannelId).filter((key) => !isNaN(Number(key))).map((key) => Number(key)).sort((a, b) => b - a);
...
task(
  ...
  action6
).addParam("networks", "Comma-separated list of networks", void 0, types.csv, true).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel).addParam(
  "json",
...
task(
  ...
  action7
).addParam("networks", "Comma-separated list of networks", void 0, types.csv, true).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel);
var action8 = async ({ logLevel = "info", oappConfig }, hre) => {
  ...
  const applicative = createDefaultApplicative(logger4);
...
task(
  ...
  action8
).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel).addParam("oappConfig", "Path to your LayerZero OApp config", void 0, types.string);
var OAppReadOmniGraphHardhatSchema = createOmniGraphHardhatSchema(
...
var createEndpointImportDeclaration = () => factory.createImportDeclaration(
  void 0,
  factory.createImportClause(
    false,
    void 0,
    factory.createNamedImports([
      factory.createImportSpecifier(false, void 0, factory.createIdentifier(ENDPOINT_ID))
    ])
  ),
    ...
    const connections = await Promise.all(
      Array.from(contractMap).filter(([toNetwork]) => fromNetwork !== toNetwork).map(
        ([toNetwork, toContract]) => createDefaultConfig(fromNetwork, toNetwork).then(
          (defaultConfig) => factory.createObjectLiteralExpression([
...
var generateLzConfig = async (selectedNetworks, contractName) => factory.createNodeArray([
...
var generateReadLzConfig = async (selectedNetworks, contractName) => factory.createNodeArray([
  createEndpointImportDeclaration(),
  ...createContractVariables(selectedNetworks, contractName),
...
${printJson(eidsByNetworkName)}`);
  ...
  const sourceFile = createSourceFile(oappConfig, "", ScriptTarget.ESNext, true, ScriptKind.TS);
  const generatedLzConfig = await generateReadLzConfig(selectedNetworks, contractName);
  const layerZeroConfigContent = printer.printList(ListFormat.MultiLine, generatedLzConfig, sourceFile);
  writeFileSync(oappConfig, layerZeroConfigContent);
  return oappConfig;
};
task(TASK_LZ_OAPP_READ_CONFIG_INIT, "Initialize an OApp configuration file", action11).addParam("oappConfig", "Path to the new LayerZero OApp config", void 0, types.string).addParam("contractName", "Name of contract in deployments folder", void 0, types.string).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel);
var action12 = async ({
  graph,
...
subtask(SUBTASK_LZ_OAPP_READ_WIRE_CONFIGURE, "Create a list of OmniTransactions that configure your OAppRead", action12).addParam("graph", "Configuration graph", void 0, types.any).addParam("configurator", "Configuration function matching the SDK factory", void 0, types.any, true).addParam("sdkFactory", "SDK factory for an OmniSDK", void 0, types.any, true);
...
task(TASK_LZ_OAPP_READ_WIRE, "Wire LayerZero OApp", action13).addParam("oappConfig", "Path to your LayerZero OApp config", void 0, types.string).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel).addParam(
  "loadConfigSubtask",
  "Override the default config loading subtask",
  SUBTASK_LZ_OAPP_CONFIG_LOAD,
  types.string,
  ...
  "assert",
  "Will not execute any transactions and fail if there are any transactions required to configure the OApp"
).addFlag("safe", "Use gnosis safe to sign transactions").addParam("signer", "Index or address of signer", void 0, types.signer, true);
  ...
  try {
    ...
    const enforcedOptsNetworkMatrix = points.map((row) => {
      const connectionsForCurrentRow = points.reduce((tableRow, column) => {
        ...
        return {
          ...tableRow,
          [column.networkName]: printRecord$1(enforcedOptsByMsgType)
        };
      }, {});
      return connectionsForCurrentRow;
    });
...
task(TASK_LZ_OAPP_ENFORCED_OPTS_GET, "Outputs OApp enforced options", action14).addParam("oappConfig", "Path to your LayerZero OApp config", void 0, types.string).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel);
...
task(TASK_LZ_OAPP_CONFIG_INIT, "Initialize an OApp configuration file", action15).addParam("oappConfig", "Path to the new LayerZero OApp config", void 0, types.string).addParam("contractName", "Name of contract in deployments folder", void 0, types.string).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel);
var action16 = async ({
  configPath,
  schema,
  task: task16
  ...
  logger: createModuleLogger(`${task16}${SUBTASK_LZ_OAPP_CONFIG_LOAD}`)
})({ configPath });
subtask(SUBTASK_LZ_OAPP_CONFIG_LOAD, "Loads and transforms OmniGraphHardhat into an OmniGraph", action16).addParam("configPath", "Path to the config file", void 0, types.string).addParam("schema", "Zod schema used to validate the config", void 0, types.any).addParam("task", "Task that is calling this subtask", void 0, types.string);
...
var OwnableOmniGraphHardhatSchema = createOmniGraphHardhatSchema(
...
task(TASK_LZ_OWNABLE_TRANSFER_OWNERSHIP, "Transfer ownable contract ownership", action17).addParam("oappConfig", "Path to your LayerZero OApp config", void 0, types.string).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel).addFlag("ci", "Continuous integration (non-interactive) mode. Will not ask for any input from the user").addFlag("safe", "Use gnosis safe to sign transactions").addFlag("dryRun", "Will not execute any transactions").addFlag(
  "assert",
  "Will not execute any transactions and fail if there are any transactions required to transfer the ownership"
).addParam("signer", "Index or address of signer", void 0, types.signer, true);
var createSingleViewFunctionCallerEVMFactory = (providerFactory = createProviderFactory()) => pMemoize(async (eid) => new SingleViewFunctionCallerEVM(eid, await providerFactory(eid)));
var createComputerEVMFactory = (providerFactory = createProviderFactory()) => pMemoize(async (eid) => new ComputerEVM(eid, await providerFactory(eid)));
var createCommandResolverFactory = (singleViewFunctionCallerEVMFactory = createSingleViewFunctionCallerEVMFactory(), computerEVMFactory = createComputerEVMFactory()) => pMemoize(async () => new CommandResolver(singleViewFunctionCallerEVMFactory, computerEVMFactory));
var createTimeMarkerResolverChainFactory = (providerFactory = createProviderFactory()) => pMemoize(async (eid) => new EVMTimeMarkerResolverChain(eid, await providerFactory(eid)));
var createTimeMarkerResolverFactory = (timeMarkerResolverChainFactory = createTimeMarkerResolverChainFactory()) => pMemoize(async () => new TimeMarkerResolver(timeMarkerResolverChainFactory));
var createTimeMarkerValidatorChainFactory = (providerFactory = createProviderFactory()) => pMemoize(async (eid) => new EVMTimeMarkerValidatorChain(eid, await providerFactory(eid)));
var createTimeMarkerValidatorFactory = (timeMarkerValidatorChainFactory = createTimeMarkerValidatorChainFactory()) => pMemoize(async () => new TimeMarkerValidator(timeMarkerValidatorChainFactory));
...
var action18 = async ({ command, logLevel = "info" }, _hre) => {
  ...
  logger4.info(`Decoded command: ${printJson(decodedCommand)}`);
  const { timestampTimeMarkers, blockNumberTimeMarkers } = await commandResolverSdk.extractTimeMarkers(command);
  logger4.verbose(`Extracted time markers: ${printJson({ timestampTimeMarkers, blockNumberTimeMarkers })}`);
  const resolvedTimestampTimeMarkers = await timeMarkerResolverSdk.resolveTimestampTimeMarkers(timestampTimeMarkers);
  logger4.info(`Resolved timestamp time markers: ${printJson(resolvedTimestampTimeMarkers)}`);
...
task(TASK_LZ_READ_RESOLVE_COMMAND, "Resolve an LzRead command", action18).addParam("command", "Command to resolve", void 0, types.string, false).addParam("logLevel", "Logging level. One of: error, warn, info, verbose, debug, silly", "info", types.logLevel);
