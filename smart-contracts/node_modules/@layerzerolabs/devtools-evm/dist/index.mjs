...
import { printJson, createModuleLogger } from '@layerzerolabs/io-devtools';
import { defaultAbiCoder } from '@ethersproject/abi';
import { z } from 'zod';
import { isBigNumberish, BigNumber } from '@ethersproject/bignumber/lib/bignumber';
import { formatOmniPoint, OmniSignerBase } from '@layerzerolabs/devtools';
...
var makeZeroAddress = (address) => address != null ? address : AddressZero;
var addChecksum = (address) => getAddress(address);
...
var BigNumberishSchema = z.custom((value) => isBigNumberish(value));
var BigNumberishBigIntSchema = BigNumberishSchema.transform(BigNumber.from).transform((bn) => bn.toBigInt());
var BigNumberishNumberSchema = BigNumberishSchema.transform(BigNumber.from).transform((bn) => bn.toNumber());
...
var formatOmniContract = (contract) => `EVM contract at ${formatOmniPoint(omniContractToPoint(contract))}`;
var OmniSDK = class {
  constructor(contract, logger = createModuleLogger(
...
var createProviderFactory = (urlFactory) => pMemoize(async (eid) => new JsonRpcProvider(await urlFactory(eid)));

// src/signer/sdk.ts
var import_safe_core_sdk_types = __toESM(require_src());
var OmniSignerEVMBase = class extends OmniSignerBase {
...
var GnosisOmniSignerEVM = class extends OmniSignerEVMBase {
  constructor(eid, signer, safeUrl, safeConfig, chainId, ethAdapter = new EthersAdapter({
    ethers,
    signerOrProvider: signer
  }), apiKit = new SafeApiKit({ txServiceUrl: safeUrl, apiKey: safeConfig.safeApiKey, chainId }), safeSdkPromise = Safe.create({
    ethAdapter,
...
var createAnvilCliOptions = ({
  ...
  [
    pipe(
      O.fromNullable(host),
      O.map((host2) => ["--host", host2])
    ),
    pipe(
      O.fromNullable(port),
      O.map((port2) => ["--port", String(port2)])
    ),
    pipe(
      O.fromNullable(mnemonic),
      O.map((mnemonic2) => ["--mnemonic", mnemonic2])
    ),
    pipe(
      O.fromNullable(count),
      O.map((count2) => ["--count", String(count2)])
    ),
    pipe(
      O.fromNullable(derivationPath),
      O.map((derivationPath2) => ["--derivation-path", derivationPath2])
    ),
    pipe(
      O.fromNullable(forkUrl),
      O.map((forkUrl2) => ["--fork-url", forkUrl2])
    ),
    pipe(
      O.fromNullable(forkBlockNumber),
      O.map((forkBlockNumber2) => ["--fork-block-number", String(forkBlockNumber2)])
    ),
    pipe(
      O.fromNullable(retries),
      O.map((retries2) => ["--retries", String(retries2)])
    ),
    pipe(
      O.fromNullable(timeout),
      O.map((timeout2) => ["--timeout", String(timeout2)])
    ),
    pipe(
      O.fromNullable(blockTime),
      O.map((blockTime2) => ["--block-time", String(blockTime2)])
    ),
    pipe(
      O.fromNullable(state),
      O.map((state2) => ["--state", state2])
    ),
    pipe(
      O.fromNullable(stateInterval),
      O.map((stateInterval2) => ["--state-interval", String(stateInterval2)])
    ),
    pipe(
...
export { BigNumberishBigIntSchema, BigNumberishNumberSchema, BigNumberishSchema, ContractError, CustomError, GnosisOmniSignerEVM, OmniSDK, OmniSignerEVM, OmniSignerEVMBase, PanicError, RevertError, UnknownError, addChecksum, connectOmniContract, createAnvilCliOptions, createContractErrorParser, createProviderFactory, formatOmniContract, isEVMAddress, makeZeroAddress, omniContractToPoint, parseContractError, parseGenericError, parseLogs, parseLogsWithName };
