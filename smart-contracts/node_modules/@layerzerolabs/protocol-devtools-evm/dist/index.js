...
var Uln302UlnConfigSchema = protocolDevtools.Uln302UlnConfigSchema.extend({
  ...
  optionalDVNThreshold: zod.z.number(),
  requiredDVNCount: zod.z.number()
});
...
var Uln302 = class extends devtoolsEvm.OmniSDK {
  ...
  encodeExecutorConfig(config) {
    ...
    return assert3__default.default(typeof encoded === "string", "Must be a string"), encoded;
  }
...
var BlockedUln302 = class extends Uln302 {
  async getAppUlnConfig(eid, address, _type) {
    this.logger.verbose(
      `Returning blocked config for ULN configs for eid ${eid} (${devtools.formatEid(eid)}) and OApp ${address} because the message library is BlockedMessageLib`
    );
    return {
      confirmations: BigInt(0),
      requiredDVNs: [],
  ...
  async hasAppUlnConfig(eid, oapp, _config, type) {
    this.logger.verbose(
      `Skipping check for ULN ${type} configs for eid ${eid} (${devtools.formatEid(eid)}) and OApp ${oapp} because the message library is BlockedMessageLib`
    );
    return true;
  }
  ...
  async getAppExecutorConfig(eid, address) {
    this.logger.verbose(
      `Returning blocked config for Executor configs for eid ${eid} (${devtools.formatEid(eid)}) and OApp ${address} because the message library is BlockedMessageLib`
    );
    return {
  ...
  async hasAppExecutorConfig(eid, oapp, _config) {
    this.logger.debug(
      `Skipping check for Executor configs for eid ${eid} (${devtools.formatEid(eid)}) and OApp ${oapp} because the message library is BlockedMessageLib`
    );
    return true;
  }
};
...
var EndpointV2 = class extends devtoolsEvm.OmniSDK {
  ...
  async isBlockedLibrary(uln) {
    assert3__default.default(!devtools.isZero(this.contract.contract.address), "EndpointV2 can not be zero");
    try {
      return await this.contract.contract.blockedLibrary() === devtoolsEvm.addChecksum(uln);
    } catch (error) {
...
exports.BlockedUln302 = BlockedUln302;
exports.DVN = DVN;
