    ...
    withOAppLogger(
      async ({ vector: { from, to }, config }, sdk) => {
        ...
        if (currentSendLibrary === void 0) {
          ...
          logger.verbose(
            `Current sendLibrary could not be determined for ${formatOmniVector({ from, to })}. This might mean BlockedMessageLib is already set. Desired library: ${desiredLibrary}`
          );
    ...
    withOAppLogger(
      async ({ vector: { from, to }, config }, sdk) => {
        ...
        if (currentReceiveLibrary === void 0) {
          ...
          logger.verbose(
            `Current receiveLibrary could not be determined for ${formatOmniVector({ from, to })}. This might mean BlockedMessageLib is already set. Desired library: ${desiredLibrary}`
          );
          let isSettingBlockedLib = false;
          try {
            isSettingBlockedLib = await endpointSdk.isBlockedLibrary(desiredLibrary);
