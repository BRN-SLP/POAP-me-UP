...
var sendBatchedIfAvailable = (fallbackLogic) => async (eid, logger, signer, transactions, onSuccess, onError) => {
  ...
  for (let i = 0; i < transactions.length; i += batchSize) {
    ...
    const transactionsName = ioDevtools.pluralizeNoun(batch.length, `1 transaction`, `${batch.length} transactions`);
    try {
...
${ioDevtools.printJson(transactions)}`);
  ...
  while (true) {
    ...
    penguinSpinner = swag.render(
      swag.createPenguinSpinner({ before: "Signing... ", after: ` 0/${transactionsToSign.length}` })
    );
    let successfulBatch = [];
    let errorsBatch = [];
    let pendingBatch = [];
